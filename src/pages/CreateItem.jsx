...
const CreateItem = () => {
  ...
  const showMessageBanner = (message, statusType) => {
    setFields(true);
    setMsg(message);
    setAlertStatus(statusType);
    setTimeout(() => {
      setFields(false);
      statusType === "danger" && setIsLoading(false);
    }, 4000);
  };
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          setIsLoading(false);
          showMessageBanner("Image uploaded successfully 🎉", "success");
        });
      }
    );
  };
  ...
  const deleteImage = () => {
    ...
    deleteObject(deleteRef).then(() => {
      setImageAsset(null);
      setIsLoading(false);
      showMessageBanner("Image deleted successfully 🎉", "success");
    });
  };
  ...
  const saveDetails = () => {
    ...
    try {
      if (!title || !calories || !imageAsset || !price || !category) {
        showMessageBanner("Required fields can't be empty", "danger");
      } else {
        const data = {
          id: `${Date.now()}`,
          title: title,
        ...
        saveItem(data);
        setIsLoading(false);
        showMessageBanner("Data Uploaded succesfullu 🎉", "success");
        clearData();
      }
    } catch (error) {
      console.log(error);
